...
function free_check(){
	...
	$block_file=array();
	for ($i=$free_START;$i<=$free_END;$i++){
		$buffer=file_get_contents($block_NAME[$i]);
		$buffer=explode(", ",$buffer);
	...
	for ($i=0;$i<sizeof($block_free);$i++) {
		if ($block_free[$i]==0 and !(in_array($block_free[$i], $block_file))){
			fwrite($log,"ERROR: BLOCK $buffer[$i] is free block but not in the free block list!\n");
			$block_file[]=$block_free[$i];
